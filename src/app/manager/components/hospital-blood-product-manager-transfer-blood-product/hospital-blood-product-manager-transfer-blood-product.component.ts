...
import { HospitalBloodProductTransferTableService } from '../../services/hospital-blood-product-transfer-table.service';

@Component({
  selector: 'app-hospital-blood-product-manager-transfer-blood-product',
  templateUrl: './hospital-blood-product-manager-transfer-blood-product.component.html',
  styleUrls: ['./hospital-blood-product-manager-transfer-blood-product.component.scss'],
  providers: [HospitalBloodProductTransferTableService]
})
export class HospitalBloodProductManagerTransferBloodProductComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public hospitalBloodProductTransferTableService: HospitalBloodProductTransferTableService
  ) {
    const navigation = this.router.getCurrentNavigation();
    if (navigation.extras.state) {
      const { bloodProducts } = navigation.extras.state;
      if (bloodProducts) {
        this.hospitalBloodProductTransferTableService.setDataRows(bloodProducts);
      }
    }
  }
  ...
  selectBloodProduct(bloodProduct: BloodProduct) {
    ...
    this.authService.getMyUserInfo()
      .subscribe((user: User) => {
        ...
        this.hospitalBloodProductTransferTableService.addRawDataRow(bloodProduct);
  ...
  removeBloodProductFromList(id: string) {
    this.hospitalBloodProductTransferTableService.removeDataRow(id);
    this.datatable.refresh();
  }

  transferBloodProducts() {
    const bloodProductIds = this.hospitalBloodProductTransferTableService.getAllRowIds();
  ...
  resetForm() {
    ...
    this.organizationForm.reset();
    this.transferForm.reset();
    this.hospitalBloodProductTransferTableService.removeAllDataRows();
    this.datatable.refresh();
  }
