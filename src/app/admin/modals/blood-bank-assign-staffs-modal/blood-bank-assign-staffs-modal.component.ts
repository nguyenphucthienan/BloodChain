...
@Component({
  selector: 'app-blood-bank-assign-staffs-modal',
  templateUrl: './blood-bank-assign-staffs-modal.component.html',
  styleUrls: ['./blood-bank-assign-staffs-modal.component.scss']
})
export class BloodBankAssignStaffsModalComponent implements OnInit {

  @Input() bloodBankId: string;
  @Input() rowData: TableRow;

  @Output() bloodBankStaffUpdated = new EventEmitter();
  ...
  ngOnInit() {
    ...
    this.bloodBankService.getStaffsOfBloodBank(this.bloodBankId)
  ...
  assignStaffs() {
    this.userService.assignOrganization(
      ...
      (result: any) => {
        this.bloodBankStaffUpdated.emit(result);
